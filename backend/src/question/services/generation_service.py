...
class QuestionGenerationService:
    ...
    async def get_failed_batches_for_retry(self, quiz_id: UUID) -> list[dict[str, Any]]:
        """
        Get detailed information about failed batches for retry scenarios.
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                if (
                    quiz.generation_metadata
                    and "failed_batches" in quiz.generation_metadata
                ):
                    failed_batch_keys = quiz.generation_metadata["failed_batches"]

                    # Get detailed information for each failed batch
                    generation_attempts = quiz.generation_metadata.get(
                        "generation_attempts", []
                    )
                    latest_attempt = (
                        generation_attempts[-1] if generation_attempts else {}
                    )
                    batch_results = latest_attempt.get("batch_results", {})

                    for batch_key in failed_batch_keys:
                        batch_info = batch_results.get(batch_key, {})
                        failed_batches.append(
                            {
                                "batch_key": batch_key,
                                "module_id": batch_info.get("module_id"),
                                "module_name": batch_info.get("module_name", "Unknown"),
                                "question_type": batch_info.get("question_type"),
                                "target_count": batch_info.get("target_count", 0),
                                "generated_count": batch_info.get("generated_count", 0),
                                "status": batch_info.get("status", "failed"),
                                "error": batch_info.get("error", "Unknown error"),
                            }
                        )

                logger.info(
                    "failed_batches_retrieved",
                    quiz_id=str(quiz_id),
