...
def test_create_user(db: Session) -> None:
    ...
    decrypted_access = crud.get_decrypted_access_token(user)
...
def test_get_user_by_canvas_id(db: Session) -> None:
    ...
    user_2 = crud.get_user_by_canvas_id(session=db, canvas_id=canvas_id)
    assert user_2
    assert user.canvas_id == user_2.canvas_id
    assert user.id == user_2.id

    # Compare decrypted tokens instead of encrypted ones
    assert crud.get_decrypted_access_token(user) == crud.get_decrypted_access_token(
        user_2
    )
    assert crud.get_decrypted_refresh_token(user) == crud.get_decrypted_refresh_token(
        user_2
    )
...
def test_update_user_tokens(db: Session) -> None:
    ...
    updated_user = crud.update_user_tokens(
        session=db,
        user=user,
    ...
    assert crud.get_decrypted_access_token(updated_user) == new_access
    assert crud.get_decrypted_refresh_token(updated_user) == new_refresh
...
def test_clear_user_tokens(db: Session) -> None:
    ...
    # Verify tokens exist
    assert crud.get_decrypted_access_token(user) == "test_token"
    assert crud.get_decrypted_refresh_token(user) == "test_refresh"
...
def test_token_encryption_integration(db: Session) -> None:
    ...
    assert crud.get_decrypted_access_token(user) == original_access
    assert crud.get_decrypted_refresh_token(user) == original_refresh
...
def test_get_user_by_id(db: Session) -> None:
    ...
    user_by_id = crud.get_user_by_id(session=db, user_id=user.id)
    assert user_by_id
    ...
    non_existent_user = crud.get_user_by_id(session=db, user_id=uuid.uuid4())
...
def test_user_name_persistence(db: Session) -> None:
    ...
    retrieved_user = crud.get_user_by_canvas_id(session=db, canvas_id=canvas_id)
...
def test_user_name_updates_with_token_operations(db: Session) -> None:
    ...
    # Update tokens
    updated_user = crud.update_user_tokens(
        session=db, user=user, access_token="new_token", refresh_token="new_refresh"
    )
    ...
    # Clear tokens
    cleared_user = crud.clear_user_tokens(session=db, user=updated_user)
