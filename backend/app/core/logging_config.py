...
def _add_request_context(event_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    request_method: ContextVar[str] = ContextVar("request_method", default="")
    request_path: ContextVar[str] = ContextVar("request_path", default="")

    if request_id.get():
        event_dict["request_id"] = request_id.get()
    if request_method.get():
        event_dict["method"] = request_method.get()
    if request_path.get():
        event_dict["path"] = request_path.get()
...
def _add_user_context(event_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    canvas_id: ContextVar[int] = ContextVar("canvas_id", default=0)

    if user_id.get():
        event_dict["user_id"] = user_id.get()
    if canvas_id.get():
        event_dict["canvas_id"] = canvas_id.get()
