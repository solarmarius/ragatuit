...
test.describe("Difficulty Feature Integration Tests", () => {
  ...
  test("should display difficulty selector with correct options", async ({ page }) => {
    ...
    await expect(page.locator('[data-part="content"]:visible [data-part="item"][data-value="medium"]')).toBeVisible()
    await expect(page.locator('[data-part="content"]:visible [data-part="item"][data-value="hard"]')).toBeVisible()
  })
  ...
  test("should allow switching between difficulty levels", async ({ page }) => {
    ...
    await expect(page.locator('label').filter({ hasText: 'Difficulty' }).first()).toBeVisible()
  })

  test("should prevent duplicate question type and difficulty combinations", async ({ page }) => {
    await page.goto("/create-quiz")
  ...
  test("should persist difficulty selection across modules", async ({ page }) => {
    await page.goto("/create-quiz")
    ...
    await expect(page.locator('label').filter({ hasText: 'Difficulty' })).toHaveCount(2)
  })

  test("should include difficulty in total question calculation", async ({ page }) => {
    await page.goto("/create-quiz")
  ...
  test("should display appropriate help text about difficulty", async ({ page }) => {
    await page.goto("/create-quiz")
  ...
  test("should handle difficulty in quiz creation API payload", async ({ page }) => {
    ...
    expect(capturedRequestBody.selected_modules["173467"].question_batches[0].difficulty).toBe("hard")
  })
})
