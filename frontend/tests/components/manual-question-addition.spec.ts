...
test.describe("Manual Question Addition Feature", () => {
  ...
  test.describe("Add Question Button Visibility", () => {
    test("should show Add Question button in ready_for_review status", async ({
      ...
      await page.route(
        `**/api/v1/questions/${mockQuizId}/stats`,
        async (route) => {
          await route.fulfill({
            status: 200,
            contentType: "application/json",
      ...
      await expect(addQuestionButton).toHaveCSS(
        "background-color",
  ...
  test.describe("Question Type Selection Dialog", () => {
    ...
    test("should display all 5 question types in selection step", async ({
      ...
      await expect(
        page.getByText("Choose the correct answer from 4 options")
      ).toBeVisible();
      await expect(
        page.getByText("Simple true or false statements")
      ).toBeVisible();
      await expect(
        page.getByText("Complete sentences with missing words")
      ).toBeVisible();
    });

    test("should show question type selection instructions", async ({
      page,
    }) => {
      const addQuestionButton = page.getByRole("button", {
        name: "Add Question",
      ...
      await expect(
        page.getByText("Choose the type of question you want to create.")
      ).toBeVisible();
    });
    ...
    test("should proceed to question creation when type is selected", async ({
      ...
      await expect(
        page.getByRole("heading", { name: "Create Multiple Choice Question" })
      ).toBeVisible();
    ...
    test("should allow navigation back to question type selection", async ({
      ...
      await expect(
        page.getByRole("heading", { name: "Create True/False Question" })
      ).toBeVisible();

      // Click back button
      await page
        .getByRole("button", { name: "← Back to Question Types" })
        .click();
  ...
  test.describe("Question Creation Workflow", () => {
    ...
    test("should display Multiple Choice question form with all required fields", async ({
      ...
      await expect(
        page.locator('textarea[placeholder*="question text"]')
      ).toBeVisible();
      await expect(
        page.locator('input[placeholder*="option A"]')
      ).toBeVisible();
      await expect(
        page.locator('input[placeholder*="option B"]')
      ).toBeVisible();
      await expect(
        page.locator('input[placeholder*="option C"]')
      ).toBeVisible();
      await expect(
        page.locator('input[placeholder*="option D"]')
      ).toBeVisible();
    ...
    test("should enable save button when all required fields are filled", async ({
      ...
      await page
        .locator('textarea[placeholder*="question text"]')
