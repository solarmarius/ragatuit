...
test.describe("ModuleSelectionStep Component", () => {
  ...
  test("should display course modules correctly", async ({ page }) => {
    ...
    // Check module IDs are displayed
    await expect(page.locator("text=Module ID: 173467")).toBeVisible()
    await expect(page.locator("text=Module ID: 173468")).toBeVisible()
    await expect(page.locator("text=Module ID: 173469")).toBeVisible()
  })
  ...
  test("should show visual feedback for selected modules", async ({ page }) => {
    ...
    await expect(moduleCard).toHaveCSS("background-color", "rgb(219, 234, 254)") // blue.50
    await expect(moduleCard).toHaveCSS("border-color", "rgb(59, 130, 246)") // blue.500
  })
  ...
  test("should handle empty modules response", async ({ page }) => {
    ...
    // Mock empty modules API response
    await page.route(
      "**/api/v1/canvas/courses/37823/modules",
      async (route) => {
        await route.fulfill({
  ...
  test("should show warning when no course is selected", async ({ page }) => {
    // Navigate directly to create quiz page without selecting course
    await page.goto("/create-quiz")

    // Go to step 2 (module selection) without selecting a course
    await page.evaluate(() => {
      // This simulates accessing step 2 without proper course selection
      window.history.pushState({}, "", "/create-quiz?step=2")
    })

    await page.reload()

    // Should show warning message
    await expect(page.locator("text=No course selected")).toBeVisible()
    await expect(
      page.locator(
        "text=Please go back to the previous step and select a course first",
      ),
    ).toBeVisible()
  })
  ...
  test("should allow checkbox interaction independent of card clicks", async ({
    ...
    // Click checkbox directly (not the card)
    await page
      .locator('[data-testid="module-card-173467"] input[type="checkbox"]')
      .click()
