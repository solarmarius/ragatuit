...
test.describe("ManualModuleDialog Component - Basic UI Tests", () => {
  ...
  test("should navigate to file upload step when PDF option is selected", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("button:has-text('Process Content')"),
    ).toBeVisible()
  })

  test("should navigate to text input step when text option is selected", async ({
    page,
  }) => {
  ...
  test("should handle back navigation from file upload step", async ({
    page,
  }) => {
  ...
  test("should handle back navigation from text input step", async ({
    page,
  }) => {
  ...
  test("should handle text input in content area", async ({ page }) => {
    ...
    await expect(textarea).toHaveValue(
      "This is test content for the manual module.",
    )
  })
  ...
  test("should disable process button when form is incomplete", async ({
    page,
  }) => {
    // Navigate to text input step
    await page.getByRole("button", { name: "Enter Text Content" }).click()
  ...
  test("should show form elements in text input step", async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="Enter"]')).toHaveValue(
      "Test Module",
    )
