...
test.describe("Navigation", () => {
  ...
  test("should navigate to dashboard from any page", async ({ page }) => {
    ...
    await expect(activeElement).toHaveCSS("background-color", "rgb(255, 255, 255)");
  });
  ...
  test("should show quizzes link in sidebar", async ({ page }) => {
    ...
    expect(currentUrl.includes('/quiz') || currentUrl.includes('/')).toBeTruthy();
  });
  ...
  test("should navigate back and forward using browser buttons", async ({ page }) => {
    // Navigate to settings
    await page.click('a[href="/settings"]');
    await expect(page).toHaveURL("/settings");
